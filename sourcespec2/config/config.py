...
class Config(dict):
    ...
    def validate(self):
        ...
        self._check_non_negative_list('geom_spread_n_exponents')
        self._check_non_negative_list('geom_spread_n_distances')
        self._check_min_max_pair('spectral_sn_freq_range')
        self._check_min_max_pair('fc_min_max')
        self._check_min_max_pair('pi_fc_min_max')
        self._check_min_max_pair('pi_ssd_min_max')
    ...
    def _check_non_negative_list(self, param):
        """
        Check that all values in a float list parameter are non-negative.

        :raises ValueError: If a negative value is found
        """
        values = self[param]
        if values is None:
            return
    ...
    def _check_min_max_pair(self, param, min_val=None, max_val=None):
        """
        ...
        :raises ValueError: If the parameter is not a valid min/max pair
        """
        values = self[param]
        if values is None:
            return
...
def _init_traceid_map(config):
    ...
    try:
        ...
        sys.exit(
            f'traceid mapping file "{config.traceid_mapping_file}" not found '
