...
def _read_geotiff(tif_file, grayscale=False):
    ...
    :param tif_file: GeoTIFF file to read.
    :type tif_file: str
    :param grayscale: read the GeoTIFF as grayscale.
    :type grayscale: bool

    :return: tif_raster and bounding box.
    :rtype: tuple of numpy.ndarray, tuple of float
    """
...
def _add_geotiff(ax):
    ...
    :param ax: axes to plot the geotiff.
    :type ax: matplotlib.axes.Axes
    """
...
def _make_geoaxes_planar(fig, bounding_box, maxdiagonal):
    """
...
def _make_station_map(
        basemap_data, lonlat_dist, st_ids, values, outliers,
        vmean, verr, vname):
    """
    Make a map of stations with the given values.

    :param basemap_data: dictionary with the basemap data.
    :param lonlat_dist: array with the station coordinates and distances.
    :type lonlat_dist: numpy.ndarray
    :param st_ids: list with the station IDs.
    :type st_ids: list of str
...
def plot_stations(sspec_output):
    ...
    try:
        ...
        basemap_data_fc = _make_basemap(maxdist)
    _make_station_map(
